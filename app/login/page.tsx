...
import { Loader2, Mail, Lock, Zap, Users, BarChart3, GitBranch } from 'lucide-react'
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'hsl(222 47% 6%)' }}>
      {/* Left panel */}
      <div className="hidden lg:flex flex-col justify-between w-1/2 p-12" style={{
        background: 'linear-gradient(135deg, hsl(239 84% 10%) 0%, hsl(222 47% 8%) 100%)',
        ...
        <div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            ...
            <p className="text-lg mb-12" style={{ color: 'hsl(215 20% 65%)' }}>
              Real-time kanban, subtasks, comments, and live collaboration — all in one beautiful workspace.
            </p>
          </motion.div>
            ...
            {FEATURES.map((f, i) => (
              ...
              >
                <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0"
                  ...
                  <f.icon className="w-5 h-5" style={{ color: 'hsl(239 84% 72%)' }} />
                </div>
                <div>
                  <div className="font-medium text-white text-sm">{f.label}</div>
                  <div className="text-xs" style={{ color: 'hsl(215 15% 45%)' }}>{f.desc}</div>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        ...
        >
          ...
          <p className="mb-8" style={{ color: 'hsl(215 20% 65%)' }}>Sign in to your workspace</p>

          {/* Demo quick-login */}
          <div className="mb-6 p-4 rounded-xl" style={{ background: 'hsl(222 35% 12%)', border: '1px solid hsl(222 25% 18%)' }}>
            <div className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: 'hsl(215 15% 45%)' }}>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white"
                      style={{ background: u.name === 'Sahinur Rahman' ? '#6366f1' : u.name === 'Om Prakash' ? '#10b981' : '#f59e0b' }}>
                      {u.name.split(' ').map(n => n[0]).join('')}
                    </div>
                    <div>
                      <div className="text-sm font-medium text-white">{u.name}</div>
                      <div className="text-xs" style={{ color: 'hsl(215 15% 45%)' }}>{u.email}</div>
                    </div>
                  </div>
                  <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(99,102,241,0.15)', color: 'hsl(239 84% 72%)' }}>
                    {u.role}
                  </span>
                </button>
              ))}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: 'hsl(215 15% 45%)' }} />
                <input
                  {...register('email')}
                  type="email"
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: 'hsl(215 15% 45%)' }} />
                <input
                  {...register('password')}
                  type="password"
                  placeholder="••••••••"
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, hsl(239 84% 67%), hsl(270 84% 67%))' }}
            >
          ...
          <p className="mt-6 text-center text-xs" style={{ color: 'hsl(215 15% 45%)' }}>
            Default password for all demo accounts: <code className="text-indigo-400">password123</code>
          </p>
        </motion.div>
      </div>
    </div>
